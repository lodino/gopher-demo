...
def preprocess_german(df, preprocess):
    if preprocess:
        ...
        df['credit_hist'] = df['credit_hist'].map({'A30': 'no credits taken/ all credits paid back duly',
                                                   'A31': 'all credits at this bank paid back duly',
                                                   'A32': 'existing credits paid back duly till now',
                                                   'A33': 'delay in paying off in the past',
                                                   'A34': 'critical account/ other credits existing'})

        df['savings'] = df['savings'].map({'A61': '< 100 DM', 'A62': '100 ~ 500 DM',
                                           'A63': '500 ~ 1000 DM', 'A64': '>= 1000 DM',
                                           'A65': 'unknown/ no savings account'})
        df['employment'] = df['employment'].map({'A71': 'unemployed', 'A72': '< 1 year',
                                                 'A73': '1 ~ 4 years', 'A74': '4 ~ 7 years',
                                                 'A75': '>= 7 years'})
        df['gender'] = df['personal_status'].map({'A91': 'male', 'A92': 'female', 'A93': 'male',
                                                  'A94': 'male', 'A95': 'female'})
        df['debtors'] = df['debtors'].map({'A101': 'none', 'A102': 'co-applicant', 'A103': 'guarantor'})
        df['property'] = df['property'].map({'A121': 'real estate',
                                             'A122': 'building society savings agreement/ life insurance',
                                             'A123': 'car or other', 'A124': 'unknown / no property'})
        df['install_plans'] = df['install_plans'].map({'A141': 'bank', 'A142': 'stores', 'A143': 'none'})
        ...
        df['age'] = df['age'].apply(lambda x: '>= 45' if x >= 45 else '< 45')  # 1 if old, 0 if young
        df['amt'] = 0
        df.loc[(df['credit_amt'] <= 2000), 'amt'] = '<= 2000'
        df.loc[(df['credit_amt'] > 2000) & (df['credit_amt'] <= 5000), 'amt'] = '2000 ~ 5000'
        df.loc[(df['credit_amt'] > 5000), 'amt'] = '> 5000'
        df['credit_amt'] = df['amt'].copy()
        df['dur'] = 0
        df.loc[(df['duration'] <= 12), 'dur'] = '<= 12'
        df.loc[(df['duration'] > 12) & (df['duration'] <= 24), 'dur'] = '12 ~ 24'
        df.loc[(df['duration'] > 24) & (df['duration'] <= 36), 'dur'] = '24 ~ 36'
        df.loc[(df['duration'] > 36), 'dur'] = '> 36'
        df['duration'] = df['dur'].copy()
        df['job'] = df['job'].map({'A171': 'unemployed/ unskilled - non-resident',
                                   'A172': 'unskilled - resident', 'A173': 'skilled employee / official',
                                   'A174': 'management/ self-employed/highly qualified employee/ officer'})
        df['telephone'] = df['telephone'].map({'A191': 'not registered', 'A192': 'registered'})
        df['foreign_worker'] = df['foreign_worker'].map({'A201': 'yes', 'A202': 'no'})

        df.drop(columns=['amt', 'dur'])
...
def load_german(preprocess=True):
    ...
    df = pd.read_table(join(dirname(__file__), 'german.data'), names=cols, sep=" ", index_col=False)
...
def load_adult(sample=False):
    ...
    if sample:
        df_train = pd.read_csv(join(dirname(__file__), 'adult-sample-train-10pc'), names=cols, sep=",")
        df_test = pd.read_csv(join(dirname(__file__), 'adult-sample-test-10pc'), names=cols, sep=",")
    else:
        df_train = pd.read_csv(join(dirname(__file__), 'adult.data'), names=cols, sep=",")
        df_test = pd.read_csv(join(dirname(__file__), 'adult.test'), names=cols, sep=",")
...
def load_compas():
    df = pd.read_csv(join(dirname(__file__), 'lib/compas-scores-two-years.csv'))
...
def load_traffic():
    df = pd.read_csv(join(dirname(__file__), 'traffic_violations_cleaned.csv'))
    y = df['search_outcome']
...
def load_sqf():
    df_train = pd.read_csv(join(dirname(__file__), 'sqf_train.csv'))
    y_train = df_train['frisked']
    X_train = df_train.drop(columns=['frisked'])
    
    df_test = pd.read_csv(join(dirname(__file__), 'sqf_test.csv'))
...
def load(dataset, preprocess=True, row_num=10000, attr_num=30, sample=False):
    if dataset == 'compas':
        ...
        return load_german(preprocess=preprocess)
