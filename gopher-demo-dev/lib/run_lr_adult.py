...
for i in range(len(candidates_1)):
    if float(candidates_1[i][2]) >= support:  # if score > top-k, keep in candidates, not otherwise
        candidates_all.append(candidates_1[i][:-1])
...
candidates_df_3_compact = pd.DataFrame(candidates_support_3_compact,
                                       columns=["predicates", "support", "score", "2nd-inf"])
...
containment_df = candidates_df_3_compact[candidates_df_3_compact['support'] \
    < 30].sort_values(by=['score'], ascending=False).copy()
...
explanations = pd.DataFrame(expl, columns=["explanations", "support", "score", 
...
print(explanations.sort_values(by=['score'], ascending=False))
explanations.sort_values(by=['score'], ascending=False).to_csv('lr_explanation_result.csv', index=False)
