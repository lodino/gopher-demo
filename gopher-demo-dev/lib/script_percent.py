...
from classifier import LogisticRegression, NeuralNetwork, SVM
from utils import *
...
for i in range(1, 26):
    ...
    for r in range(repeat):
        ...
        clf.fit(np.array(X_train), np.array(y_train), use_sklearn=True)
        t0 = time.time()
        X = np.delete(X_train, idx, 0)
        y = y_train.drop(index=idx, inplace=False)
        inf_gt = 0
        clf.fit(np.array(X), np.array(y), use_sklearn=True)
    ...
    pers.append([time_first, time_second, time_gt, time_gd])

np.save('pers.npy', np.array(pers))